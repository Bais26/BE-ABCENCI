...
def get_all_karyawan(
    ...
    query = db.query(User).options(
        joinedload(User.karyawan_detail)  # ✅ Load relationship
    ).filter(User.role == UserRole.KARYAWAN)
...
def get_karyawan_by_id(
    karyawan_id: UUID,
    db: Session = Depends(get_db),
    # current_user: User = Depends(get_current_user),  # ✅ Uncomment untuk protect
):
    """
    Get specific karyawan by ID with full details
    """
